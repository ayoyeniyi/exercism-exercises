...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const remainderRate = remainder * dayRate(ratePerHour);
  const monthlyRate = 22 * dayRate(ratePerHour);
  const discountedMonthlyRate = monthlyRate * (1 - discount);
  const totalMonthlyRate = discountedMonthlyRate * fullMonths;

  return Math.ceil(totalMonthlyRate + remainderRate);
